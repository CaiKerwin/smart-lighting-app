<template>
	<view :class="themeClass" class="lightIntensityTimeTable-container">
		<!-- ==================== 时间表名称介绍 ==================== -->
		<view class="page-header">
			...
			<view class="header-meta">
				<text class="meta-item">按环境照度调节亮度</text>
			...
			<view class="scroll-inner">
				...
				<template v-else>
					<view
						...
						:key="index"
						:class="['period-card', isEnabled(period.mode) ? '' : 'card-disabled']"
					>
						<!-- 卡片头部：时段序号 + 模式 -->
						<view class="card-head">
							...
							<text :class="['mode-chip', isEnabled(period.mode) ? 'chip-enable' : 'chip-disabled']">
								{{ modeText(period.mode) }}
							</text>
						</view>

						<!-- 参数区：照度(lx) / 方式 / 亮度(%) / 联动亮度(%)，两行两列 -->
						<view class="field-grid">
							<!-- 照度阈值 -->
							<view class="field">
								<text class="field-label">照度</text>
								<view class="value-box">
									<text class="value-text">{{ displayValue(period.value) }}</text>
							...
							<view class="field">
								...
								<view class="value-box">
									<text class="value-text">{{ typeText(period.type) }}</text>
								</view>
							</view>
							<!-- 亮度 / 联动亮度 -->
					...
					<view class="scroll-footer-tip">
						环境照度满足「方式 + 照度」条件时，把灯具亮度调整为设定值；联动亮度用于联动调节。点击右下角按钮可编辑
					</view>
				</template>
			</view>
		</scroll-view>
...
// 每个时段展示的百分比参数：亮度(%)、联动亮度(%)
const FIELD_KEYS = ['bright', 'union'];

// 字段展示信息
const FIELD_LABELS = {
	bright: {label: '亮度', unit: '%'},
	union: {label: '联动亮度', unit: '%'}
};

// 选项数组中查找值对应的选项（找不到返回 null）
...
export default {
	...
	data() {
		return {
			...
			timeTableId: null,
			timeTableName: '',
			// 6 个时段参数（只读展示）：{ mode, value(照度), type(方式), bright, union }
	...
	onLoad(options) {
		...
		uni.$on('lightIntensityTimeTableUpdated', this.refreshHandler);

		this.getTimeTableDetail();
	},
	onUnload() {
		if (this.refreshHandler) {
			uni.$off('lightIntensityTimeTableUpdated', this.refreshHandler);
	...
	methods: {
		...
		// 是否为启用模式
		isEnabled(mode) {
			return Number(mode) === MODE_ENABLE;
		},

		// 模式文案
		modeText(mode) {
			if (mode === null || mode === undefined || mode === '') return '--';
			const option = findOption(MODE_OPTIONS, mode);
			return option ? option.label : '未启用';
		},
				...
				if (payload && payload.data) {
					try {
						...
						for (let i = 0; i < PERIOD_COUNT; i++) {
							...
							const period = {};
							['mode', 'value', 'type'].concat(FIELD_KEYS).forEach((key) => {
		...
		editTimeTable() {
			...
			uni.navigateTo({
				url: `/pages/timeTable/components/timeTableEdit/editLightIntensityTimeTable?id=${this.timeTableId}&name=${encodeURIComponent(this.timeTableName || '')}`
			});
		}
	}
}
...
/* ==================== 容器：头部 + 滚动区 ==================== */
.lightIntensityTimeTable-container {
	display: flex;
