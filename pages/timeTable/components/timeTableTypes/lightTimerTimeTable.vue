<template>
	<view :class="themeClass" class="lightTimerTimeTable-container">
		<!-- ==================== 时间表名称介绍 ==================== -->
				...
				<template v-else>
					<view v-for="(period, index) in periods" :key="index" class="period-card">
						...
						<view class="card-head">
							...
							<text class="duration-chip">{{ displayValue(period.min) }} min</text>
						</view>
					...
					<view class="scroll-footer-tip">
						每个时段仅一组参数：时长(min)、亮度(%)、色温(%)、联动亮度(%)，点击右下角按钮可编辑
					</view>
				</template>
			</view>
		</scroll-view>
...
export default {
	...
	onLoad(options) {
		...
		uni.$on('lightTimerTimeTableUpdated', this.refreshHandler);

		this.getTimeTableDetail();
	},
	onUnload() {
		if (this.refreshHandler) {
			uni.$off('lightTimerTimeTableUpdated', this.refreshHandler);
	...
	methods: {
					...
					try {
						...
						for (let i = 0; i < PERIOD_COUNT; i++) {
							...
							list.push({
								...
								ch1: source.ch1 === undefined || source.ch1 === null ? '' : source.ch1,
								c1: source.c1 === undefined || source.c1 === null ? '' : source.c1,
								un1: source.un1 === undefined || source.un1 === null ? '' : source.un1
							});
						}
						this.periods = list;
		...
		editTimeTable() {
			...
			uni.navigateTo({
				url: `/pages/timeTable/components/timeTableEdit/editLightTimerTimeTable?id=${this.timeTableId}&name=${encodeURIComponent(this.timeTableName || '')}`
			});
		}
	}
}
