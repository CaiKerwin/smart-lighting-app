<template>
	<view :class="themeClass" class="page-wrapper">
		<!-- 工单列表 -->
		<view :class="{ 'select-mode-active': isSelectMode }" class="false-feedback-list-container">
			<view v-for="(item, index) in falseFeedbackListData"
				  :key="index"
				  class="list-item"
			...
			>
				...
				<view :class="{ 'select-mode': isSelectMode }" class="card">
					...
					<view class="card-left">
						<image class="card-icon" mode="aspectFit" src="/static/workOrder/feedback.png"></image>
					</view>
...
export default {
	...
	onLoad() {
		// 加载时获取误报反馈列表数据
		this.getFalseFeedbackListData();
	},
	computed: {
		// 判断是否已全选
		isAllSelected() {
			return this.falseFeedbackListData.length > 0 && this.falseFeedbackListData.every(item => this.selectedIds.includes(item.id));
		}
	},
	methods: {
		getFalseFeedbackListData() {
			/**
			 * [
			 *   {
			...
			request({
				...
				uni.hideLoading();
				console.error('获取误报反馈工单列表错误', err.message);
		...
		handleItemClick(item) {
			if (this.isSelectMode) {
				...
				this.goToFalseFeedbackWODetail(item.id);
			}
		},
		toggleSelect(item) {
		...
		toggleSelectAll() {
			if (this.isAllSelected) {
				...
				this.selectedIds = this.falseFeedbackListData.map(item => item.id);
			}
		},
		exitSelectMode() {
			this.selectedIds = [];
				...
				success: (res) => {
					if (res.confirm) {
						this.batchDeleteWorkOrder(this.selectedIds)
							.then(() => {
								this.falseFeedbackListData = this.falseFeedbackListData.filter(
