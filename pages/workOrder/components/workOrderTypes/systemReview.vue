<template>
	<view :class="themeClass" class="page-wrapper">
		<!-- 工单列表 -->
		<view :class="{ 'select-mode-active': isSelectMode }" class="system-review-list-container">
			<view v-for="(item, index) in systemReviewListData"
				  :key="index"
				  class="list-item"
...
export default {
	...
	onLoad() {
		// 加载时获取系统审核工单列表数据
		this.getSystemReviewListData();
	},
	computed: {
		// 判断是否已全选
		isAllSelected() {
			return this.systemReviewListData.length > 0 && this.systemReviewListData.every(item => this.selectedIds.includes(item.id));
		}
	},
	methods: {
		getSystemReviewListData() {
			/**
			 * [
			 *   {
			...
			request({
				...
				if (payload && payload.data) {
					const systemReviewWOData = JSON.parse(base64Decode(payload.data));
					this.systemReviewListData = systemReviewWOData.map((item,index) =>({
						time: item.fireTime || '',
					...
					uni.showToast({title: '获取系统审核工单列表数据失败', icon: 'none'});
				}
				if (this.systemReviewListData.length === 0){
					uni.showToast({title: '暂无系统审核工单', icon: 'none'})
				}
				this.exitSelectMode();
			}).catch(err =>{
				uni.hideLoading();
				console.error('获取系统审核工单列表错误', err.message);
		...
		handleItemClick(item) {
			if (this.isSelectMode) {
				...
				this.goToSystemReviewWODetail(item.id);
			}
		},
		toggleSelect(item) {
		...
		toggleSelectAll() {
			if (this.isAllSelected) {
				...
				this.selectedIds = this.systemReviewListData.map(item => item.id);
			}
		},
		exitSelectMode() {
			this.selectedIds = [];
				...
				success: (res) => {
					if (res.confirm) {
						this.batchDeleteWorkOrder(this.selectedIds)
							.then(() => {
								this.systemReviewListData = this.systemReviewListData.filter(
