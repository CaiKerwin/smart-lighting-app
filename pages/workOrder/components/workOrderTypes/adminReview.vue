<template>
	<view class="page-wrapper">
		<!-- 工单列表 -->
		<view :class="{ 'select-mode-active': isSelectMode }" class="admin-review-list-container">
			<view v-for="(item, index) in adminReviewListData"
				  :key="index"
				  class="list-item"
			...
			>
				...
				<view :class="{ 'select-mode': isSelectMode }" class="card">
					...
					<view class="card-left">
						<image class="card-icon" mode="aspectFit" src="/static/workOrder/admin.png"></image>
					</view>
...
export default {
	...
	computed: {
		// 判断是否已全选
		isAllSelected() {
			return this.adminReviewListData.length > 0 && this.adminReviewListData.every(item => this.selectedIds.includes(item.id));
		}
	},
	methods: {
		getAdminReviewListData() {
			...
			request({
				...
				if (payload && payload.data) {
					const adminReviewWOData = JSON.parse(base64Decode(payload.data));
					this.adminReviewListData = adminReviewWOData.map((item,index) =>({
						time: item.fireTime || '',
				...
				uni.hideLoading();
				console.error('获取管理员审核工单列表错误', err.message);
				uni.showToast({ title: '网络异常，请重试', icon: 'none' });
			})
		},
		goToAdminReviewWODetail(id) {
			uni.navigateTo({
		...
		// ----- 点击工单（选择模式切换选中，否则跳转详情） -----
		handleItemClick(item) {
			if (this.isSelectMode) {
				this.toggleSelect(item);
			} else {
				this.goToAdminReviewWODetail(item.id);
			}
		},
		...
		toggleSelectAll() {
			if (this.isAllSelected) {
				...
				this.selectedIds = this.adminReviewListData.map(item => item.id);
			}
		},
				...
				success: (res) => {
					if (res.confirm) {
						this.batchDeleteWorkOrder(this.selectedIds)
							.then(() => {
								this.adminReviewListData = this.adminReviewListData.filter(
