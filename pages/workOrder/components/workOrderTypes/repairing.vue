<template>
	<view :class="themeClass" class="page-wrapper">
		<!-- 工单列表 -->
		<view :class="{ 'select-mode-active': isSelectMode }" class="repairing-list-container">
			<view v-for="(item, index) in repairingListData"
				  :key="index"
				  class="list-item"
			...
			>
				...
				<view :class="{ 'select-mode': isSelectMode }" class="card">
					...
					<view class="card-left">
						<image class="card-icon" mode="aspectFit" src="/static/workOrder/processing.png"></image>
					</view>
...
export default {
	...
	onLoad() {
		this.getRepairingListData();
	},
	computed:{
		// 判断是否已全选
		isAllSelected() {
			return this.repairingListData.length > 0 && this.repairingListData.every(item => this.selectedIds.includes(item.id));
		}
	},
	methods: {
		getRepairingListData() {
			/**
			 * [
			 *   {
			...
			request({
				...
				if (payload && payload.data) {
					const repairingWOData = JSON.parse(base64Decode(payload.data));
					this.repairingListData = repairingWOData.map((item,index) =>({
						time: item.fireTime || '',
				...
				uni.hideLoading();
				console.error('获取维修中工单列表错误', err.message);
		...
		handleItemClick(item) {
			if (this.isSelectMode) {
				...
				this.goToRepairingWODetail(item.id);
			}
		},
		toggleSelect(item) {
		...
		toggleSelectAll() {
			if (this.isAllSelected) {
				...
				this.selectedIds = this.repairingListData.map(item => item.id);
			}
		},
		exitSelectMode() {
			this.selectedIds = [];
				...
				success: (res) => {
					if (res.confirm) {
						this.batchDeleteWorkOrder(this.selectedIds)
							.then(() => {
								this.repairingListData = this.repairingListData.filter(
