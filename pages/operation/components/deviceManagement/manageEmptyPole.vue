<template>
	<view :class="themeClass" class="page-container">
		...
		<!-- ==================== 编辑空灯杆弹窗（与灯杆管理共用 PoleInfo，接口 /station/config/SaveLampPole） ==================== -->
...
export default {
	...
	methods: {
		...
		getStationPoleList() {
			this.loading = true;
			request({
				url: '/station/config/QueryNoLampPoleByFilter',
				method: 'POST',
				data: {
				...
				this.total = 0;
				this.poleList = [];
				console.error('获取站点空灯杆列表失败', err && err.message);
				uni.showToast({ title: '获取空灯杆列表失败', icon: 'none' });
			}).finally(() => {
				// 停止下拉刷新动画
				setTimeout(() => {
		...
		parseResponseData(data) {
			...
			try {
				...
				console.error('解析空灯杆列表数据失败', e && e.message);
				return null;
			}
		},
		 ...
		 */
		onPoleSaved(payload) {
			// 新增的灯杆不一定在当前页，回到第一页查询（本页面无新增入口，保留兼容）
		...
		deletePole(item) {
			uni.showModal({
				title: '删除空灯杆',
				content: `确定要删除 ${item.name || '该空灯杆'} 吗？`,
				success: (res) => {
					if (res.confirm) {
						request({
							...
							uni.showToast({ title: '删除失败', icon: 'none' });
							console.error('删除空灯杆失败', err.message);
						}).finally(() =>{
							this.getStationPoleList();
						})
					}
...
/* ==================== 页面容器 ====================
 * 头部搜索栏与底部分页器固定，仅中间的空灯杆卡片列表随页面滚动
 */
.page-container {
	position: relative;
	min-height: 100vh;
	...
	/* 下：让开固定底栏（分页器） */
	padding: 160rpx 20rpx calc(220rpx + env(safe-area-inset-bottom));
